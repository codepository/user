...
// FindAllUserInfoLimit 查询所有用户
func FindAllUserInfoLimit(fields string, query interface{}, limit, offset int) ([]*Userinfo, error) {
	var users []*Userinfo
	if len(fields) == 0 {
		fields = "*"
	}
	...
	err := wxdb.Table(UserinfoTabel).Select(fields).Where(query).
...
// FindSingleUserByUserid 根据id查询用户
func FindSingleUserByUserid(id int) (*Userinfo, error) {
	var user *Userinfo
	err := wxdb.Table(UserinfoTabel).Where(id).Find(&user).Error
	return user, err
}
